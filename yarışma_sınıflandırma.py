...
model_for_kanama = tf.keras.models.load_model('C://Users//samet//OneDrive//Masaüstü//teknofest_test//modeller//kanama_eski_weihts.h5', custom_objects = dependencies)
model_for_iskemi = tf.keras.models.load_model("C://Users//samet//OneDrive//Masaüstü//teknofest_test//modeller//iskemi_eski_weihts.h5", custom_objects = dependencies)
...
for x in range(cropped_img.shape[0]):
    ...
    pred_kanama = cv2.morphologyEx(pred_kanama[0], cv2.MORPH_OPEN, kernel)

    pred_iskemi = model_for_iskemi.predict(örnek)
    pred_iskemi = cv2.morphologyEx(pred_iskemi[0], cv2.MORPH_OPEN, kernel)

    pred = pred_iskemi + pred_kanama
    
    pred = np.expand_dims(pred,axis=0)
    
    pred[pred>(pred[0].max()*63/100)]=255.0
    pred[pred<(pred[0].max()*63/100)]=0.0
    
    pred=pred.reshape(1,img_rows,img_cols)
    seg_preds.append(pred)
    
seg_preds = np.array(seg_preds,dtype="float32")

#%%
list_of_value=[]
for img in seg_preds:
    value=0
    for x in range(len(img[0])):
        for y in range(len(img[0])):
             value+=img[0][x][y]
    
    list_of_value.append(int(value/255))


classes_for_predict=[]
for value in list_of_value:
    if value>55:       
        classes_for_predict.append(1)
    else:
        classes_for_predict.append(0)
...
summary["ID"] = ids
summary["ETİKET"] = classes_for_predict

summary.to_csv("OTURUM1.csv",index=False)
